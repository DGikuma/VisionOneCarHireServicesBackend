...
/**
 * @swagger
 * /api/agent_bookings/send-confirmation:
 *   post:
...
router.get('/', (req: Request, res: Response) => {
    res.json({
        message: 'Vision Wan Car Hire Agent Booking API',
        version: '1.1.0',
        endpoints: [
            { method: 'POST', path: '/api/agent_bookings',                   description: 'Submit new booking with documents' },
            { method: 'POST', path: '/api/agent_bookings/lookup',            description: 'Look up a booking by ID or email' },
            { method: 'POST', path: '/api/agent_bookings/amend',             description: 'Amend an existing booking' },
            { method: 'POST', path: '/api/agent_bookings/send-confirmation', description: 'Resend confirmation email' },
            { method: 'GET',  path: '/api/agent_bookings/download-excel',    description: 'Download bookings Excel file' },
            { method: 'GET',  path: '/api/agent_bookings/health',            description: 'Check API health status' },
        ],
        status: 'operational',
        timestamp: new Date().toISOString(),
    });
...
/**
 * @swagger
 * /api/agent_bookings/download-excel:
 *   get:
